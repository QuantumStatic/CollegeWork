...
class BinarySearchTree:
        ...
        for num in newElements:
            ...
            while True:
                if num == iterator.value:
                    ...
                    if iterator.right is not eval(type(self).__name__).sentinel:
                        iterator = iterator.right
                    else:
                        iterator.right = self.leaf(parent=iterator, value=num)
                        break
                else:
                    if iterator.left is not eval(type(self).__name__).sentinel:
    ...
    def remove(self, toRemove):
        ...
        if toDelete.repititions:
            ...
            return temp
        if toDelete.left is eval(type(self).__name__).sentinel:
            return self.transplant(toDelete, toDelete.right)
        elif toDelete.right is eval(type(self).__name__).sentinel:
    ...
    def rotate(self, parentLeaf, left=False, right=False):
        ...
        if (left and right) or (not left and not right):
            ...
            if parentLeaf.left == eval(type(self).__name__).sentinel:
                raise Exception("Type None rotations are not supported")
            leftChild, parentLeaf.left = parentLeaf.left, parentLeaf.left.right
            if leftChild.right != eval(type(self).__name__).sentinel:
    ...
    def height(tree):
        if tree is None:
            return 1
        return int(max(BinarySearchTree.height(tree.left), BinarySearchTree.height(tree.right)) + 1)
...
class RedBlackTree(BinarySearchTree):
    ...
    sentinel = leaf(parent=None, value=None, colour=BLACK, rightChild=None, leftChild= None)

    def __init__(self, *args):
        elements = RedBlackTree.breaker(args)
        self.root = self.leaf(value=elements[0], parent=RedBlackTree.sentinel, rightChild=RedBlackTree.sentinel, leftChild=RedBlackTree.sentinel, colour=BLACK)
        del elements[0]
        self.insert(elements)
            ...
            while True:
                if num == iterator.value:
                    ...
                    if iterator.right != RedBlackTree.sentinel:
                        ...
                        iterator.right = self.leaf(parent=iterator, value=num, rightChild=RedBlackTree.sentinel, leftChild=RedBlackTree.sentinel)
                    ...
                    if iterator.left != RedBlackTree.sentinel:
                        ...
                        iterator.left = self.leaf(parent=iterator, value=num,rightChild=RedBlackTree.sentinel, leftChild=RedBlackTree.sentinel)
...
class SplayTree(BinarySearchTree):
    ...
    def __init__(self,*args):
            ...
            self.root = self.leaf(value=elements[rootVal], parent=SplayTree.sentinel, rightChild=SplayTree.sentinel, leftChild=SplayTree.sentinel)
    ...
    def insert(self, *toAdd):
        ...
        for newElement in newElements:
            rightHeight, leftHeight, toSplay = SplayTree.height(self.root.right), SplayTree.height(self.root.left), self.leaf
            if rightHeight > leftHeight:
                toSplay = self.greatestMin(newElement)
            elif leftHeight > rightHeight:
                toSplay = self.smallestMax(newElement)
            else:
                toSplay = self.greatestMin(newElement) if choice([False, True]) else self.smallestMax(newElement)
            if toSplay.value != newElement:
                splayed, newRoot = self.splay(toSplay), self.leaf()
                if toSplay.value > newElement:
                    newRoot = self.leaf(parent=SplayTree.sentinel, rightChild=splayed, leftChild=splayed.left, value=newElement)
                    splayed.left.parent, splayed.left = newRoot, SplayTree.sentinel  
                else:
                    newRoot = self.leaf(parent=SplayTree.sentinel, rightChild=splayed.right, leftChild=splayed, value=newElement)
                    splayed.right.parent, splayed.right = newRoot, SplayTree.sentinel
                splayed.parent = self.root = newRoot  

    def delete(self, toDelete):
        nodeToDelete, newRoot, rightHeight, leftHeight, chc = self.splay(toDelete), self.leaf, SplayTree.height(self.root.right), SplayTree.height(self.root.left), None
        if rightHeight > leftHeight:
            chc = True
            ...
            chc = choice([False, True])
            newRoot = self.next(nodeToDelete, returnNode=True) if chc else self.prev(nodeToDelete, returnNode=True)
...
class Treap(BinarySearchTree):
    class leaf(Node):
        __slots__ = ['parent', 'right', 'left', 'value', 'repititions', 'priority']
    ...
    def __init__(self, values:Iterable, priorities:Iterable):
        if priorities is None:
            priorities, values = tuple(sample(range(len(values)**2),k=len(values))), tuple(values)
        elements = Treap.make_pair(values, priorities)
        if isinstance(elements,zip):
            for index, pair in enumerate(elements):
                if index == 0:
                    self.root = self.leaf(value = pair[0], parent=Treap.sentinel, rightChild=Treap.sentinel, leftChild=Treap.sentinel, priority=pair[1])
                else:
                    self.insert(pair)
        del elements
        ...
        while True:
                if element[0] == iterator.value and element[1] == iterator.priority:
                    ...
                    if iterator.right != Treap.sentinel:
                        ...
                        iterator.right = self.leaf(parent=iterator, value=element[0], rightChild=Treap.sentinel, leftChild=Treap.sentinel, priority=element[1])
                        self.insertionFixup(iterator.right)
                        break
                else:
                    if iterator.left != RedBlackTree.sentinel:
                        ...
                        iterator.left = self.leaf(parent=iterator, value=element[0],rightChild=Treap.sentinel, leftChild=Treap.sentinel, priority=element[1])
                        self.insertionFixup(iterator.left)
                        break 
    ...
    def deletionFixup(self, toFix):
        minPriority = min(toFix.priority, toFix.left.priority if toFix.left is not Treap.sentinel else float("-inf"), toFix.right.priority if toFix.right is not Treap.sentinel else float("-inf"))
    ...
    def make_pair(value, priority):
        if type(value) != type(priority):
            raise TypeError(f"Data type of parameters should be same. Received {type(value).__name__} and {type(priority).__name__}")
        if isinstance(value, list) or isinstance(value, tuple) or isinstance(value, set):
            return zip(Treap.breaker(value), Treap.breaker(priority))
        elif isinstance(priority,int):
            return tuple(value,priority)
        else:
            raise TypeError(f"{type(value)} or {type(priority)} not supported.")
