...
unordered_map<int, deque<Process>> RQs;
/*mapping all queues according to their priorities */
...
int main(int argc, const char * argv[]) {
    RQs.insert(make_pair(0,RQ0));
    RQs.insert(make_pair(1,RQ1));
    RQs.insert(make_pair(2,RQ2));
    global::populate_processes(argv[2]);
    sort(allProcesses.begin(), allProcesses.end(), compare_processes);
    global::scheduling_algo = argv[1];
    if (global::scheduling_algo.compare("FCFS") == 0)
        scheduling::FCFS();
    else if (global::scheduling_algo.compare("RR") == 0)
        scheduling::RR();
    else if (global::scheduling_algo.compare("FB") == 0)
        scheduling::FB();
    output_CPU_times(argv[2], completed_processes);
}
...
void populate_processes(string path){
    ...
    while (not inFile.eof()){
        ...
        vector<string> input_vector = global::break_input(input);           /* returns a vector after seperating input on the basis of space*/
        if (input_vector[0] == "#")
            global::add_process(input_vector);
        else
...
vector<string> break_input(string input){
    ...
    stringstream ss(input);                         // puts the input stream into string stream
    
    while (not ss.eof()){
        string word;
        ss >> word;                                 // seperates by space and puts it into the vector
...
void check_blocked_queues(deque<Process> &queue_to_check){
    if (not queue_to_check.empty() and queue_to_check.front().curr_cmd_completed()){ /* Checks if a completed command is waiting to be moved to ready queue*/
        queue_to_check.front().remove_completed_command();
        RQs[queue_to_check.front().get_priority()].push_back(queue_to_check.front()); /* moving commands based on priority */
        queue_to_check.pop_front();
    }
}
void check_blocked_queues(Mutex &mtx){          // Checking mutex's blocked queue
    mtx.check_blocked_queue();
}

void prepare_ready_queue_for_next_tick(int priority=0){
    if (not RQs[priority].empty() and (RQs[priority].front().completed() or RQs[priority].front().curr_cmd_completed())){ /* Checking a if a processes' command is completed or the process is completed*/
    
        RQs[priority].front().remove_completed_command();
        if (RQs[priority].front().completed()){
            
            RQs[priority].front().add_to_output(global::start_tick);        /* Since the process is completed, adding its starting and finishing times */
            global::start_tick = global::clock + 1;
            RQs[priority].front().add_to_output(global::clock+1);
            
            if (RQs[priority].front().is_a_command_left())
                mtx.unlock(RQs[priority].front());              /* If the last command left was U mtx*/
            ...
            char curr_cmd = RQs[priority].front().curr_command_type(); /* peaking at the next command*/
            
            if (curr_cmd == 'L'){
                RQs[priority].front().remove_completed_command();
                if (mtx.is_locked()){       /* If the mutex is locked it would be removed from the ready queue and other logistics handling */
                    RQs[priority].front().add_to_output(global::start_tick);
                    RQs[priority].front().add_to_output(global::clock+1);
                    global::start_tick = global::clock+1;
...
void add_new_arrived_proccesses(){
    while (not allProcesses.empty() and allProcesses.front().get_arrival_time() == global::clock){
    ...
    while (total_processes != completed_processes.size() or not allProcesses.empty()){
        ...
        if (RQs[0].empty()){
            ...
            curr_CPU_pid = -1;
                                                ...
                                                /* Following the steps mentioned in the PDF with the addition of checking if queue is empty*/
        tick(disk_interrupt);
        tick(keyBoard_interrupt);
        tick(RQs[0]);
        
        if (not RQs[0].empty() and curr_CPU_pid != RQs[0].front().get_pid()){
            global::start_tick = global::clock;
    ...
    if (not RQs[queue_num].empty()){
        if (pid == RQs[queue_num].front().get_pid()) {
            if (++runtime >= 5) {
                ...
                process.add_to_output(global::start_tick);
                process.add_to_output(global::clock+1);       /* if a process has finished their time quanta shift  according to the scheduling policy */
                
                RQs[queue_num].pop_front();
                
                global::start_tick = global::clock+1;
                if (global::scheduling_algo.compare("FB") == 0){
...
bool FB(){
    ...
    while (total_processes != completed_processes.size() or not allProcesses.empty()){
        ...
        if (not RQs[ticked].empty() and curr_CPU_pid[ticked] == RQs[ticked].front().get_pid())
            tick(RQs[ticked]);
        else{                           /* Ticking a process if it's already running otherwise, using the priority checking finding a process to execute. */
            ticked = 0;
            bool flag = false;
            for (int i=0; i <= 2; i++)
                if (tick(RQs[i])){
