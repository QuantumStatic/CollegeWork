...
def rando_generator():
    ...
    while True:
        yield random.gauss(mu=0.143354477*10, sigma=3.499999999999)

def generate_results():
    """
    This function generates and visualizes some statistics for a list of 
    random numbers generated by rando_generator.
    """

    # Total number of data points to generate
    total_pts = 1000
    # Initializes counter for the number of data points generated so far
    pts_in = 0
    # List to store generated data points
    pts = []

    # Generates random numbers until we reach the desired number of data points
    for random_num in rando_generator():
        if pts_in == total_pts:
            break
        # Only includes numbers greater than -9
        if random_num > -9:
            pts.append(random_num)
            pts_in += 1

    # Calculates and prints various statistics for the generated data points
    print(statistics.mean(pts))
    print(statistics.median(pts))
    print(statistics.stdev(pts))
    print(statistics.variance(pts))
    print(np.percentile(pts, 25))
    print(np.percentile(pts, 75))
    print(max(pts))
    print(min(pts))

    # Sets the plot style to whitegrid
    sns.set(style="whitegrid")
    # Creates a scatter plot of the generated data points
    sns.scatterplot(x=[i+1 for i in range(total_pts)], y=pts)
    # Adds a text label for the mean
    plt.text(1, statistics.mean(pts)+0.1, 'Mean', fontsize=10, color='red')
    # Adds a horizontal line for the mean
    plt.axhline(y=statistics.mean(pts), color='r', linestyle='dashdot')
    # Adds a horizontal line at y=0
    plt.axhline(y=0, color='black', linestyle='-')
    # Sets the y-axis ticks to include all integers from -9 to the maximum data point
    plt.yticks(ticks=[x for x in range(-9,
