...
class Model:
	...
	def __init__(self, name:str, test_accuracy:float, data_path:Path):
		...
		self._populate()
		print(self.availability, self.response_time, self.transaction_rate)
	
	def _populate(self) -> None:
		with open(self.data_path/f"single_pod_{self.data_path.parts[-1]}.txt", 'r', encoding="utf-8") as file:
			for line in file:
...
def slide2():
	...
	for model in (pred_model, acc_model, infer_model):
		ax.scatter(model.transaction_rate, model.test_accuracy, label=model.name)
	...
	x_axis = "Transaction rate"
	y_axis = "Test Accuracy"

	title = f"{y_axis} vs {x_axis} (Single Pod deployment)"
